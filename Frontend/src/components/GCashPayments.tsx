...
export default function GCashPayments({ role, onBack }: { role: 'admin' | 'client'; onBack?: () => void }) {
  ...
  return <div className="space-y-6 text-slate-900">
    ...
    {admin && <div className="grid gap-5 lg:grid-cols-2">
      <section className={card}><h2 className="font-bold">GCash via PayMongo</h2><p className="mt-3 text-sm">{configured ? 'Backend PayMongo key configured.' : 'PayMongo configuration is unavailable. Check the backend setup and restart it.'}</p><p className="mt-3 text-sm text-slate-500">Clients authorize payments on PayMongo checkout. Status is checked while this page is open or when you refresh. New payments cannot be manually marked paid.</p></section>
